"""Filing Landscape service - competitive 3x/4x/5x matrix from SEC filings.
...
def build_filing_landscape(db: Session) -> dict:
    ...
    Returns dict with:
      matrices: {3x: {underlier: {issuer: True}}, 4x: ..., 5x: ...}
      kpis: {count_3x, count_4x, count_5x, rex_exclusive, total_names}
      issuer_scorecard: [{issuer, c3, c4, c5, total, exclusive}]
      active_issuers: {3x: [...], 4x: [...], 5x: [...]}
      generated_at: str
    """
    rows = db.execute(
    ...
    total_names = len(set().union(
        *(set(matrices[lev].keys()) for lev in ("3x", "4x", "5x"))
    ))
